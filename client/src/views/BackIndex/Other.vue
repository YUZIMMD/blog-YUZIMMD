  ...
  <div class="other">
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="文章展示图片" :label-width="formLabelWidth">
          <el-upload
            class="upload-demo"
            drag
            action="http://localhost:8080/api/uploadImg/upload"
...
export default {
  ...
  methods: {
    ...
    submitFun() {
      let url =
        this.title == "新增文章"
          ? "/other/addotherAction"
          : "/other/updateotherAction";
    ...
    init() {
      this.$http("get", "/other/queryotherAction").then(data => {
        this.tableData = data.otherlist.map(item => {
          if (item.createTime) {
    ...
    deleteUser(row) {
      this.$http("get", "/other/deleteotherAction?id=" + row.id).then(data => {
        if (data.code == 200) {
          this.init();
        }
      });
